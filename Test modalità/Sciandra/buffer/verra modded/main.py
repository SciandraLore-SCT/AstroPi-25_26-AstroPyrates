...
class ISS_SpeedCalculator:
    ...
    def kalman_filter_fusion(self, measurement_photo, measurement_gyro,
        ...
        if len(measurements) == 1:
            ...
            state_estimate = predicted_state + K * (z - predicted_state)
            state_variance = (1 - K) * predicted_variance
        ...
        state_estimate = predicted_state + K @ y
        state_variance = predicted_variance - np.sum(K) * predicted_variance

        return float(state_estimate), float(state_variance)
    ...
    def esegui(self):
        ...
        state_estimate, state_variance = self.kalman_filter_fusion(
            photo_velocity,
            gyro_velocity,
            state_estimate,
            state_variance
        ...
        self.write_final_result(
            f"Velocità stimata ISS: {state_estimate:.3f} km/s"
        )
